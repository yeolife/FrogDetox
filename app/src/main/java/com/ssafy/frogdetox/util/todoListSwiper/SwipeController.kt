package com.ssafy.frogdetox.util.todoListSwiper
    ...
    private fun drawButtons(
        ...
        if (buttonsShowedState === ButtonState.RIGHT_VISIBLE) { //왼쪽으로 스와이프 했을 때, 오른쪽 버튼(삭제)이 나와야 함
            val rightButton = RectF(
                itemView.right - buttonWidthWithOutPadding, (itemView.top + 5).toFloat(),
                (itemView.right - 10).toFloat(), (itemView.bottom - 5).toFloat()
            )
            p.color = Color.parseColor("#BE4941")
